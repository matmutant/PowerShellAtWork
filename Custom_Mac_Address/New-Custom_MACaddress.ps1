####################################################################################
#.Synopsis 
#    Assigns a new custom or random MAC address to a physical network interface.
#
#.Description 
...
#
#.Parameter SpoofedMacAddress
#    If you want to set a specific MAC address, use the 12 caracters without ':' in between. Else, a random MAC address will be set.
#
#.Parameter Wireless
...
#  Modifications: matmutant (https://github.com/matmutant/PowerShellAtWork/Custom_Mac_Address)
# Version: 1.1
# Updated: 18.Jul.2014
...
param ($InterfaceIndexNumber,$SpoofedMacAddress, [Switch] $Wireless, [Switch] $DoNotResetInterface, [Switch] $ResetDefault)
...
else
{
	...
	$regpath = "hklm:\system\CurrentControlSet\control\class\{4D36E972-E325-11CE-BFC1-08002BE10318}\$index"
	# Check if a custom (and valid!) MAC address has been passed in parameter
	if ($SpoofedMacAddress -match '^[0-9A-F]{12}$')
	{
		"Custome MAC passed to script, setting custom MAC with value $SpoofedMacAddress"
		set-itemproperty -path $regpath -name "NetworkAddress" -value $SpoofedMacAddress
	}
	else
	{
		"NO custom MAC has been passed, using Random one instead "
		if ($wireless)
		{
